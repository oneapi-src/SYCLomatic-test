...
void test7() {
    ...
    cudaMemcpy(diffdata, host_diffdata.data(), sizeof(float) * oele_num,
      cudaMemcpyHostToDevice);
    cudaMemcpy(diffscale, host_diffscale.data(), sizeof(float) * sele_num,
      cudaMemcpyHostToDevice);
    cudaMemcpy(diffbias, host_diffbias.data(), sizeof(float) * sele_num,
      cudaMemcpyHostToDevice);
...
void test8() {
    ...
    cudaMemcpy(diffdata, host_diffdata.data(), sizeof(float) * oele_num,
      cudaMemcpyHostToDevice);
    cudaMemcpy(diffout, host_diffout.data(), sizeof(float) * oele_num, cudaMemcpyHostToDevice);
    cudaMemcpy(diffscale, host_diffscale.data(), sizeof(float) * sele_num, cudaMemcpyHostToDevice);
    cudaMemcpy(diffbias, host_diffbias.data(), sizeof(float) * sele_num, cudaMemcpyHostToDevice);
    float alpha = 2.5f, beta = 1.5f, eps = 1.f;
    double factor = 0.5f;
    ...
    cudaMalloc(&breservespace, reservespace_size);

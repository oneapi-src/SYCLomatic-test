// ====--------- math-emu-half2-after12.cu --------- *- CUDA -* -----------===//
//
...
__global__ void heq2_mask(unsigned *const Result, __half2 Input1,
                          __half2 Input2) {
  *Result = __heq2_mask(Input1, Input2);
}

void testHeq2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
...
__global__ void hequ2_mask(unsigned *const Result, __half2 Input1,
                           __half2 Input2) {
  *Result = __hequ2_mask(Input1, Input2);
}

void testHequ2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hge2_mask(unsigned *const Result, __half2 Input1,
                          __half2 Input2) {
  *Result = __hge2_mask(Input1, Input2);
}

void testHge2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hgeu2_mask(unsigned *const Result, __half2 Input1,
                           __half2 Input2) {
  *Result = __hgeu2_mask(Input1, Input2);
}

void testHgeu2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hgt2_mask(unsigned *const Result, __half2 Input1,
                          __half2 Input2) {
  *Result = __hgt2_mask(Input1, Input2);
}

void testHgt2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hgtu2_mask(unsigned *const Result, __half2 Input1,
                           __half2 Input2) {
  *Result = __hgtu2_mask(Input1, Input2);
}

void testHgtu2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hle2_mask(unsigned *const Result, __half2 Input1,
                          __half2 Input2) {
  *Result = __hle2_mask(Input1, Input2);
}

void testHle2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hleu2_mask(unsigned *const Result, __half2 Input1,
                           __half2 Input2) {
  *Result = __hleu2_mask(Input1, Input2);
}

void testHleu2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hlt2_mask(unsigned *const Result, __half2 Input1,
                          __half2 Input2) {
  *Result = __hlt2_mask(Input1, Input2);
}

void testHlt2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hltu2_mask(unsigned *const Result, __half2 Input1,
                           __half2 Input2) {
  *Result = __hltu2_mask(Input1, Input2);
}

void testHltu2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hne2_mask(unsigned *const Result, __half2 Input1,
                          __half2 Input2) {
  *Result = __hne2_mask(Input1, Input2);
}

void testHne2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {
...
__global__ void hneu2_mask(unsigned *const Result, __half2 Input1,
                           __half2 Input2) {
  *Result = __hneu2_mask(Input1, Input2);
}

void testHneu2_maskCases(const vector<pair<half2_pair, unsigned>> &TestCases) {
  unsigned *Result;
  cudaMallocManaged(&Result, sizeof(*Result));
  for (const auto &TestCase : TestCases) {

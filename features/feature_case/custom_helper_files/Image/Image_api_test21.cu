...
// RUN: dpct --format-range=none  --usm-level=none  --use-custom-helper=api -out-root %T/Image/api_test21_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only

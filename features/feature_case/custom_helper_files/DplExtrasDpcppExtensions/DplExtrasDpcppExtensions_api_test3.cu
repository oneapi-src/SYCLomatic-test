// ====------ DplExtrasDpcppExtensions_api_test3.cu---------- *- CUDA -* ----===////
//
...
// RUN: dpct --format-range=none  --use-custom-helper=api -out-root %T/DplExtrasDpcppExtensions/api_test3_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++14
// RUN: grep "IsCalled" %T/DplExtrasDpcppExtensions/api_test3_out/MainSourceFiles.yaml | wc -l > %T/DplExtrasDpcppExtensions/api_test3_out/count.txt

// ====------ DplExtrasDpcppExtensions_api_test2.cu---------- *- CUDA -* ----===////
//
...
// RUN: dpct --format-range=none  --use-custom-helper=api -out-root %T/DplExtrasDpcppExtensions/api_test2_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++14

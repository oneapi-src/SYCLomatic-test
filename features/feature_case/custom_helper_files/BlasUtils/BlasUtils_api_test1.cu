...
// RUN: dpct --format-range=none --use-custom-helper=api -out-root %T/BlasUtils/api_test11_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only

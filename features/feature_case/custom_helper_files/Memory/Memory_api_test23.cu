...
// RUN: dpct --format-range=none  --usm-level=none  --use-custom-helper=api -out-root %T/Memory/api_test23_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only

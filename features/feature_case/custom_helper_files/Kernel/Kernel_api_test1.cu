...
// RUN: dpct --format-range=none  --usm-level=none  --use-custom-helper=api -out-root %T/Kernel/api_test1_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only

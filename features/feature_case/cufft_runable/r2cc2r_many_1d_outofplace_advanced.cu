// ===--- r2cc2r_many_1d_outofplace_advanced.cu --------------*- CUDA -*---===//
//
...
bool r2cc2r_many_1d_outofplace_advanced() {
  ...
  cudaMalloc(&forward_idata_d, sizeof(float) * 26);
  cudaMalloc(&forward_odata_d, sizeof(float2) * 6);
  cudaMalloc(&backward_odata_d, sizeof(float) * 26);
  cudaMemcpy(forward_idata_d, forward_idata_h, sizeof(float) * 26, cudaMemcpyHostToDevice);

  long long int n[1] = {5};
  long long int inembed[1] = {12};
  long long int onembed[1] = {3};
  size_t workSize;
  cufftXtMakePlanMany(plan_fwd, 1, n, inembed, 2, 13, CUDA_R_32F, onembed, 1, 3, CUDA_C_32F, 2, &workSize, CUDA_C_32F);
  ...
  forward_odata_ref[1] =  float2{-2.5,3.44095};
  forward_odata_ref[2] =  float2{-2.5,0.812299};
  forward_odata_ref[3] =  float2{10,0};
  forward_odata_ref[4] =  float2{-2.5,3.44095};
  forward_odata_ref[5] =  float2{-2.5,0.812299};
  ...
  cufftXtMakePlanMany(plan_bwd, 1, n, onembed, 1, 3, CUDA_C_32F, inembed, 2, 13, CUDA_R_32F, 2, &workSize, CUDA_C_32F);
  cufftXtExec(plan_bwd, forward_odata_d, backward_odata_d, CUFFT_INVERSE);
  cudaDeviceSynchronize();
  float backward_odata_h[26];
  cudaMemcpy(backward_odata_h, backward_odata_d, sizeof(float) * 26, cudaMemcpyDeviceToHost);

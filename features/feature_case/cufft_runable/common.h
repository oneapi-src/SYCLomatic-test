...
template<>
bool compare(float2* ptr1, float2* ptr2, std::vector<int> indices) {
  for (int i = 0 ; i < indices.size(); i++) {
    if (std::abs(ptr1[indices[i]].x - ptr2[indices[i]].x) > 0.01 || std::abs(ptr1[indices[i]].y - ptr2[indices[i]].y) > 0.01) {
      return false;
    }
  }
  return true;
...
template<>
bool compare(double2* ptr1, double2* ptr2, std::vector<int> indices) {
  for (int i = 0 ; i < indices.size(); i++) {
    if (std::abs(ptr1[indices[i]].x - ptr2[indices[i]].x) > 0.01 || std::abs(ptr1[indices[i]].y - ptr2[indices[i]].y) > 0.01) {
      return false;
    }
...
template<>
bool compare(double2* ptr1, double2* ptr2, int ele_num) {
  for (int i = 0 ; i < ele_num; i++) {
    if (std::abs(ptr1[i].x - ptr2[i].x) > 0.01 || std::abs(ptr1[i].y - ptr2[i].y) > 0.01) {
      return false;
    }

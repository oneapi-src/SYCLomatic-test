...
__global__ void test_add_kernel(uint32_t *d_out, uint32_t a, uint32_t b) {
  *d_out = add(a, b);
}
...
bool run_test() {
  ...
  uint32_t expected = packHalfFloats(2.5, 2.5); // Expected result (2.5, 2.5)

  cudaMalloc(&d_out, sizeof(uint32_t));
  test_add_kernel<<<1, 1>>>(d_out, a, b);

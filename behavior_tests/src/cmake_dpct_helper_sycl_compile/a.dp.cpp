...
void add(int size, float *a, float *res, const sycl::nd_item<3> &item_ct1) {
...
void add_wrapper(int size, float *a, float *d, int numOfBlocks,
                 int threadsPerBlock, sycl::queue &q_ct1) {
  q_ct1.parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, numOfBlocks) *
                            sycl::range<3>(1, 1, threadsPerBlock),
                        sycl::range<3>(1, 1, threadsPerBlock)),
      [=](sycl::nd_item<3> item_ct1) { add(size, a, d, item_ct1); });
}

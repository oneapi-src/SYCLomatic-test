...
int main() {
  ...
  dpct::dpct_memcpy(array1->to_pitched_data(), sycl::id<3>(0, 0, 0), dpct::pitched_data(host_buffer, 640 * sizeof(cl::sycl::float4), 640 * sizeof(cl::sycl::float4), 1), sycl::id<3>(0, 0, 0), sycl::range<3>(640 * sizeof(cl::sycl::float4), 1, 1));
  ...
  }
  {
    ...
    dpct::get_default_queue().submit([&](cl::sycl::handler &cgh) {
      ...
      auto acc21 = static_cast<dpct::image_wrapper<cl::sycl::float4, 1> *>(tex41)->get_access(cgh);
      ...
      cgh.single_task<dpct_kernel_name<class dpct_single_kernel>>([=] {
        test_image(acc_out.get_pointer(),dpct::image_accessor_ext<cl::sycl::float4, 2>(smpl42, acc42),
                   dpct::image_accessor_ext<cl::sycl::float4, 1>(smpl21, acc21),

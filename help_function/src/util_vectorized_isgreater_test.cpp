...
void vec_max_test_1() {
  ...
  unsigned int ref_data[num_data] = {0, 0, 0, 0, 0xffff, 0xffff, 0xffff};
  for (unsigned int i = 0; i < num_data; i++) {
...
void test_kernel_3(unsigned int vect_count, unsigned int *input_1,
                   unsigned int *input_2, unsigned int *output,
                   sycl::nd_item<3> item_ct1, const sycl::stream &stream_ct1) {
  ...
  if (index < vect_count) {
    ...
    output[index] = dpct::vectorized_isgreater<sycl::uchar4, unsigned int>(
        input_1[index], input_2[index]);
...
void vec_max_test_3() {
  ...
  unsigned int *h_out_data = (unsigned int *)malloc(mem_size);
  unsigned int *h_data = (unsigned int *)malloc(mem_size);
  unsigned int a_array[num_data] = {0xff11, 0xffff, 0xee11, 3,
                                    4,         5,         6};
  unsigned int b_array[num_data] = {0xff, 0xffff, 0xff00, 3,
                                    2,         1,         0};
  ...
  q_ct1.submit([&](sycl::handler &cgh) {
    ...
    cgh.parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, 3) * sycl::range<3>(1, 1, 3),
                          sycl::range<3>(1, 1, 3)),
        [=](sycl::nd_item<3> item_ct1) {
          test_kernel_3(num_data, d_in_data_1, d_in_data_2, d_out_data,
                        item_ct1, stream_ct1);
        });
  });
  dev_ct1.queues_wait_and_throw();
  ...
  unsigned int ref_data[num_data] = {0xff00, 0x0, 0xff, 0x0, 0xff, 0xff, 0xff};
  for (unsigned int i = 0; i < num_data; i++) {
    if (h_out_data[i] != ref_data[i]){
      printf("vec_max_test_3 failed!\n");
      exit(-1);
    }
  }
...
int main() {
  std::cout << "\n===== vec_max_test_1 ====" << std::endl;
  vec_max_test_1();
  std::cout << "\n===== vec_max_test_2 ====" << std::endl;
  vec_max_test_2();
  std::cout << "\n===== vec_max_test_3 ====" << std::endl;
  vec_max_test_3();

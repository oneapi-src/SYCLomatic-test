// ====------ atomics_usm_verification.cpp---------- -*- C++ -* ----===////
//
...
int main(int argc, char **argv) {
  ...
  atom_arr = sycl::malloc_shared<int>(numData, dpct::get_default_queue());
  ...
  dpct::get_default_queue().submit([&](sycl::handler &cgh) {
    cgh.parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, numBlocks) *
                          ...
                          sycl::range<3>(1, 1, numThreads)),
        [=](sycl::nd_item<3> item_ct1) { atomicKernel(atom_arr, item_ct1); });
  });
  ...
  { sycl::free(atom_arr, dpct::get_default_queue()); }

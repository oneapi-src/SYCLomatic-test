...
segmented_sort_pairs(sycl::queue queue, int64_t nsegments, int64_t nsort,
  ...
  auto reverse_indices_ptr = sycl::malloc_device<int64_t>(numel, queue);

  auto event = queue.submit([&](sycl::handler &h) {
    h.parallel_for(
        numel, ([=](sycl::id<1> i) { reverse_indices_ptr[i] = i % nsort; }));
  });
  event.wait();
  if (algorithm == 0) {
...
int test_with_generated_offsets(const int64_t nsegments, const int64_t nsort,
  ...
  std::vector<int64_t> output_indices(n, 0);
  ...
  int64_t *dev_output_indices = sycl::malloc_device<int64_t>(numel, queue);
  ...
  queue.memcpy(output_keys.data(), dev_output_keys, n * sizeof(scalar_t))
      .wait();
  queue.memcpy(output_indices.data(), dev_output_indices, n * sizeof(int64_t))
      .wait();
  ...
  for (int seg = 0; seg < nsegments; seg++) {
    for (int i = 0; i < nsort; i++) {
      if (seg * nsort + i < n) {
        bool local =
            (input_keys[output_indices[i + seg * nsort] + seg * nsort] ==
             output_keys[i + seg * nsort]);
        if (!local) {
          std::cout << "Output index incorrect: seg = " << seg << ", i = " << i
                    << ", output_keys[i] = " << output_keys[i + seg * nsort]
                    << ", output_indices[i] = "
                    << output_indices[i + seg * nsort]
                    << " input_keys[output_indices[i]] = "
                    << input_keys[output_indices[i + seg * nsort] + seg * nsort]
                    << std::endl;
        }
        ret &= local;
...
int test_with_device_offsets(bool descending, int algorithm) {
  ...
  int *d_vals = sycl::malloc_device<int>(num_items, q);
  int *d_out_keys = sycl::malloc_device<int>(num_items, q);
  int *d_out_vals = sycl::malloc_device<int>(num_items, q);

  q.memcpy(d_keys, keys, sizeof(int) * num_items).wait();
  q.memcpy(d_vals, vals, sizeof(int) * num_items).wait();
                             ...
                             // others are for timing purposes
  {
    dpct::segmented_sort_pairs(oneapi::dpl::execution::make_device_policy(q),
                               d_keys, d_out_keys, d_vals, d_out_vals,
                               num_items, num_segs, d_offsets, d_offsets + 1,
                               descending);
  }

  q.memcpy(out_keys, d_out_keys, sizeof(int) * num_items).wait();
  q.memcpy(out_vals, d_out_vals, sizeof(int) * num_items).wait();
  ...
  if (!descending) {
    ...
    tests_failed += check_expected(1, out_vals[0]);
    tests_failed += check_expected(2, out_vals[1]);
    tests_failed += check_expected(0, out_vals[2]);
    tests_failed += check_expected(5, out_vals[3]);
    tests_failed += check_expected(4, out_vals[4]);
    tests_failed += check_expected(3, out_vals[5]);
    tests_failed += check_expected(6, out_vals[6]);
  } else {
    ...
    tests_failed += check_expected(0, out_vals[0]);
    tests_failed += check_expected(2, out_vals[1]);
    tests_failed += check_expected(1, out_vals[2]);
    tests_failed += check_expected(6, out_vals[3]);
    tests_failed += check_expected(3, out_vals[4]);
    tests_failed += check_expected(4, out_vals[5]);
    tests_failed += check_expected(5, out_vals[6]);
  }
  sycl::free(d_offsets, q);
  sycl::free(d_keys, q);
  sycl::free(d_vals, q);
  sycl::free(d_out_keys, q);
  sycl::free(d_out_vals, q);
  return tests_failed;
}

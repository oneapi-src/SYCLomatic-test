// ====------ util_shift_sub_group_left.cpp---------- -*- C++ -* ----===////
//
...
void shift_sub_group_left1(unsigned int* data, sycl::nd_item<3> item_ct1) {
  ...
  int output = 0;
  output = dpct::shift_sub_group_left(item_ct1.get_sub_group(), threadid, 1);
...
void shift_sub_group_left2(unsigned int* data, sycl::nd_item<3> item_ct1) {
  int threadid = item_ct1.get_local_id(2) +
                 ...
                 item_ct1.get_group(2) * item_ct1.get_local_range(2) *
                     item_ct1.get_local_range(1) * item_ct1.get_local_range(0);
  int output = 0;
  output = dpct::shift_sub_group_left(item_ct1.get_sub_group(), threadid, 1, 8);
  data[threadid] = output;
}
...
int main() {
  ...
  if(!verify_data<unsigned int>(dev_data_u, expect1, DATA_NUM)) {
    std::cout << "shift_sub_group_left1" << " verify failed" << std::endl;
    Result = false;
  ...
  if(!verify_data<unsigned int>(dev_data_u, expect2, DATA_NUM)) {
    std::cout << "shift_sub_group_left2" << " verify failed" << std::endl;
    Result = false;
    std::cout << "expect:" << std::endl;

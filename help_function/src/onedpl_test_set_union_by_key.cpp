// ====------ onedpl_test_set_union_by_key.cpp---------- -*- C++ -* ----===////
//
...
class SetUnionByKey {};        // name for policy
class SetUnionByKeyGreater {}; // name for policy
...
int main() {
...

    {
        ...
        sycl::buffer<uint64_t, 1> key_res_buf{ sycl::range<1>(18) };
        sycl::buffer<uint64_t, 1> val_res_buf{ sycl::range<1>(18) };
        ...
        // keys_result = { 0, 1, 1, 2, 3, 3, 4, 5, 5, 6, 7, 8, 9, 10 }
        // vals_result = { 9, 0, 1, 9, 0, 1, 9, 0, 1, 9, 0, 9, 0, 9 }
	...
	// Initialize data
        ...
        auto new_policy = oneapi::dpl::execution::make_device_policy<SetUnionByKey>(oneapi::dpl::execution::dpcpp_default);

        // call algorithm
	dpct::set_union(new_policy, key_beg1_it, key_end1_it, key_beg2_it, key_end2_it,
                               val_beg1_it, val_beg2_it, key_res_beg_it, val_res_beg_it);
	...
	// check values
        {
            ...
            test_name = "set_union_by_key test 1";

            num_failing += ASSERT_EQUAL(test_name, key_res_beg[0], 0);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[1], 1);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[2], 1);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[3], 2);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[4], 3);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[5], 3);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[6], 4);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[7], 5);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[8], 5);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[9], 6);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[10], 7);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[11], 8);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[12], 9);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[13], 10);

            num_failing += ASSERT_EQUAL(test_name, val_res_beg[0], 9);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[1], 0);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[2], 1);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[3], 9);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[4], 0);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[5], 1);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[6], 9);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[7], 0);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[8], 1);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[9], 9);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[10], 0);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[11], 9);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[12], 0);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[13], 9);
        ...
        // create named policy from existing one.
        auto new_policy2 = oneapi::dpl::execution::make_device_policy<SetUnionByKeyGreater>(oneapi::dpl::execution::dpcpp_default);

        // call algorithm
        dpct::set_union(new_policy2, key_beg1_it, key_end1_it, key_beg2_it, key_end2_it,
                               val_beg1_it, val_beg2_it, key_res_beg_it, val_res_beg_it,
	...
	// check values
        {
            ...
            test_name = "set_union_by_key test 2";
            ...
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[5], 5);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[6], 5);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[7], 4);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[8], 3);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[9], 3);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[10], 2);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[11], 1);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[12], 1);
            num_failing += ASSERT_EQUAL(test_name, key_res_beg[13], 0);
            ...
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[5], 0);
            ...
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[9], 1);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[10], 9);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[11], 0);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[12], 1);
            num_failing += ASSERT_EQUAL(test_name, val_res_beg[13], 9);

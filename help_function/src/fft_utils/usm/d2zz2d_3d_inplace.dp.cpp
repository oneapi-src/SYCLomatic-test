...
bool d2zz2d_3d_inplace() {
  ...
  double* data_d;
  data_d = sycl::malloc_device<double>(36, q_ct1);
  q_ct1.memcpy(data_d, forward_idata_h, sizeof(double) * 36).wait();
  ...
  q_ct1.memcpy(forward_odata_h, data_d, sizeof(double) * 36).wait();
  ...
  q_ct1.memcpy(backward_odata_h, data_d, sizeof(double) * 36).wait();

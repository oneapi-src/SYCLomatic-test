// ===--- r2cc2r_3d_outofplace_make_plan.dp.cpp ---------------*- C++ -*---===//
//
...
bool r2cc2r_3d_outofplace_make_plan() {
  ...
  forward_idata_d = sycl::malloc_device<float>(30, q_ct1);
  forward_odata_d = (sycl::float2 *)sycl::malloc_device(
      sizeof(sycl::float2) * (5 / 2 + 1) * 2 * 3, q_ct1);
  backward_odata_d = sycl::malloc_device<float>(30, q_ct1);
  q_ct1.memcpy(forward_idata_d, forward_idata_h, sizeof(float) * 30).wait();

  size_t workSize;
  plan_fwd->commit(&q_ct1, 2, 3, 5,
                   dpct::fft::fft_type::real_float_to_complex_float, nullptr);
  ...
  plan_bwd->commit(&q_ct1, 2, 3, 5,
                   dpct::fft::fft_type::complex_float_to_real_float, nullptr);
  ...
  q_ct1.memcpy(backward_odata_h, backward_odata_d, sizeof(float) * 30).wait();

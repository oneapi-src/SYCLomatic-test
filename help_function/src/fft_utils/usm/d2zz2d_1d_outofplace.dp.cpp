...
bool d2zz2d_1d_outofplace() {
  ...
  forward_idata_d =
      (double *)sycl::malloc_device(2 * sizeof(double) * 7, q_ct1);
  forward_odata_d = (sycl::double2 *)sycl::malloc_device(
      2 * sizeof(sycl::double2) * (7 / 2 + 1), q_ct1);
  backward_odata_d =
      (double *)sycl::malloc_device(2 * sizeof(double) * 7, q_ct1);
  q_ct1.memcpy(forward_idata_d, forward_idata_h, 2 * sizeof(double) * 7).wait();
  ...
  q_ct1.memcpy(backward_odata_h, backward_odata_d, 2 * sizeof(double) * 7)
      .wait();

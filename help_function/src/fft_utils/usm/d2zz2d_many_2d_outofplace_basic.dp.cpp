...
bool d2zz2d_many_2d_outofplace_basic() {
  ...
  forward_idata_d =
      (double *)sycl::malloc_device(sizeof(double) * 2 * 3 * 2, q_ct1);
  forward_odata_d = (sycl::double2 *)sycl::malloc_device(
      2 * 2 * sizeof(sycl::double2) * (3 / 2 + 1), q_ct1);
  backward_odata_d =
      (double *)sycl::malloc_device(sizeof(double) * 2 * 3 * 2, q_ct1);
  q_ct1.memcpy(forward_idata_d, forward_idata_h, sizeof(double) * 2 * 3 * 2)
      .wait();
  ...
  q_ct1.memcpy(backward_odata_h, backward_odata_d, sizeof(double) * 12).wait();

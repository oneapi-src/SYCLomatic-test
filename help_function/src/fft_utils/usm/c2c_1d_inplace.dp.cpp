...
bool c2c_1d_inplace() {
  ...
  q_ct1.memcpy(data_d, forward_idata_h, 2 * sizeof(sycl::float2) * 7).wait();
  ...
  q_ct1.memcpy(forward_odata_h, data_d, 2 * sizeof(sycl::float2) * 7).wait();
  ...
  q_ct1.memcpy(backward_odata_h, data_d, 2 * sizeof(sycl::float2) * 7).wait();

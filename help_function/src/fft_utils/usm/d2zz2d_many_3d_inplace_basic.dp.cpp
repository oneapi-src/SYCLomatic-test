// ===--- d2zz2d_many_3d_inplace_basic.dp.cpp -----------------*- C++ -*---===//
//
...
bool d2zz2d_many_3d_inplace_basic() {
  ...
  double* data_d;
  data_d = sycl::malloc_device<double>(64, q_ct1);
  q_ct1.memcpy(data_d, forward_idata_h, sizeof(double) * 64).wait();
  ...
  q_ct1.memcpy(forward_odata_h, data_d, sizeof(double) * 64).wait();
  ...
  q_ct1.memcpy(backward_odata_h, data_d, sizeof(double) * 64).wait();

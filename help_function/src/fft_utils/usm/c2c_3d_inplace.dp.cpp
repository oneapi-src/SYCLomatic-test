...
bool c2c_3d_inplace() {
  ...
  sycl::float2 *data_d;
  data_d = sycl::malloc_device<sycl::float2>(30, q_ct1);
  q_ct1.memcpy(data_d, forward_idata_h, sizeof(sycl::float2) * 30).wait();
  ...
  q_ct1.memcpy(forward_odata_h, data_d, sizeof(sycl::float2) * 30).wait();
  ...
  q_ct1.memcpy(backward_odata_h, data_d, sizeof(sycl::float2) * 30).wait();

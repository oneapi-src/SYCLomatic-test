...
bool d2zz2d_many_3d_outofplace_basic() {
  ...
  forward_idata_d =
      (double *)sycl::malloc_device(2 * sizeof(double) * 4 * 2 * 3, q_ct1);
  forward_odata_d = (sycl::double2 *)sycl::malloc_device(
      2 * sizeof(sycl::double2) * 4 * 2 * (3 / 2 + 1), q_ct1);
  backward_odata_d =
      (double *)sycl::malloc_device(2 * sizeof(double) * 4 * 2 * 3, q_ct1);
  q_ct1.memcpy(forward_idata_d, forward_idata_h, 2 * sizeof(double) * 4 * 2 * 3)
      .wait();
  ...
  q_ct1.memcpy(backward_odata_h, backward_odata_d, sizeof(double) * 48).wait();

...
bool r2cc2r_many_2d_outofplace_advanced() {
  ...
  forward_idata_d = sycl::malloc_device<float>(24, q_ct1);
  forward_odata_d = sycl::malloc_device<sycl::float2>(8, q_ct1);
  backward_odata_d = sycl::malloc_device<float>(24, q_ct1);
  q_ct1.memcpy(forward_idata_d, forward_idata_h, sizeof(float) * 24).wait();
  ...
  q_ct1.memcpy(forward_odata_h, forward_odata_d, sizeof(sycl::float2) * 8)
      .wait();
  ...
  q_ct1.memcpy(backward_odata_h, backward_odata_d, sizeof(float) * 24).wait();

// ===--- d2zz2d_3d_outofplace_make_plan.dp.cpp ---------------*- C++ -*---===//
//
...
bool d2zz2d_3d_outofplace_make_plan() {
  ...
  forward_idata_d = sycl::malloc_device<double>(30, q_ct1);
  forward_odata_d = (sycl::double2 *)sycl::malloc_device(
      sizeof(sycl::double2) * (5 / 2 + 1) * 2 * 3, q_ct1);
  backward_odata_d = sycl::malloc_device<double>(30, q_ct1);
  q_ct1.memcpy(forward_idata_d, forward_idata_h, sizeof(double) * 30).wait();
  ...
  q_ct1.memcpy(backward_odata_h, backward_odata_d, sizeof(double) * 30).wait();

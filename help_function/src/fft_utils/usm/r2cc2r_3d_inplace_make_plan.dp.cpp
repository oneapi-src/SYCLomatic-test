// ===--- r2cc2r_3d_inplace_make_plan.dp.cpp ------------------*- C++ -*---===//
//
...
bool r2cc2r_3d_inplace_make_plan() {
  ...
  float* data_d;
  data_d = sycl::malloc_device<float>(36, q_ct1);
  q_ct1.memcpy(data_d, forward_idata_h, sizeof(float) * 36).wait();
  ...
  q_ct1.memcpy(forward_odata_h, data_d, sizeof(float) * 36).wait();
  ...
  q_ct1.memcpy(backward_odata_h, data_d, sizeof(float) * 36).wait();

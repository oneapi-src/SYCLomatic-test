// ===--- r2cc2r_many_2d_inplace_basic.dp.cpp -----------------*- C++ -*---===//
//
...
bool r2cc2r_many_2d_inplace_basic() {
  ...
  float* data_d;
  data_d = sycl::malloc_device<float>(16, q_ct1);
  q_ct1.memcpy(data_d, forward_idata_h, sizeof(float) * 16).wait();
  ...
  q_ct1.memcpy(forward_odata_h, data_d, sizeof(float) * 16).wait();
  ...
  q_ct1.memcpy(backward_odata_h, data_d, sizeof(float) * 16).wait();

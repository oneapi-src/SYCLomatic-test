...
bool c2c_3d_outofplace() {
  ...
  forward_idata_d =
      (sycl::float2 *)dpct::dpct_malloc(sizeof(sycl::float2) * 30);
  forward_odata_d =
      (sycl::float2 *)dpct::dpct_malloc(sizeof(sycl::float2) * 30);
  backward_odata_d =
      (sycl::float2 *)dpct::dpct_malloc(sizeof(sycl::float2) * 30);
  dpct::dpct_memcpy(forward_idata_d, forward_idata_h, sizeof(sycl::float2) * 30,
                    dpct::host_to_device);

  plan_fwd = dpct::fft::fft_engine::create(
      &q_ct1, 2, 3, 5, dpct::fft::fft_type::complex_float_to_complex_float);
  ...
  dpct::dpct_memcpy(forward_odata_h, forward_odata_d, sizeof(sycl::float2) * 30,
                    dpct::device_to_host);
  ...
  dpct::dpct_memcpy(backward_odata_h, backward_odata_d,
                    sizeof(sycl::float2) * 30, dpct::device_to_host);

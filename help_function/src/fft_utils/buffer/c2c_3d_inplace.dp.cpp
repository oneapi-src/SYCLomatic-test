...
bool c2c_3d_inplace() {
  ...
  sycl::float2 *data_d;
  data_d = (sycl::float2 *)dpct::dpct_malloc(sizeof(sycl::float2) * 30);
  dpct::dpct_memcpy(data_d, forward_idata_h, sizeof(sycl::float2) * 30,
                    dpct::host_to_device);
  ...
  dpct::dpct_memcpy(forward_odata_h, data_d, sizeof(sycl::float2) * 30,
                    dpct::device_to_host);
  ...
  dpct::dpct_memcpy(backward_odata_h, data_d, sizeof(sycl::float2) * 30,
                    dpct::device_to_host);

...
bool c2c_many_1d_inplace_advanced() {
  ...
  sycl::float2 *data_d;
  data_d = (sycl::float2 *)dpct::dpct_malloc(sizeof(sycl::float2) * 18);
  dpct::dpct_memcpy(data_d, forward_idata_h, sizeof(sycl::float2) * 18,
                    dpct::host_to_device);
  ...
  dpct::dpct_memcpy(forward_odata_h, data_d, sizeof(sycl::float2) * 18,
                    dpct::device_to_host);
  ...
  dpct::dpct_memcpy(backward_odata_h, data_d, sizeof(sycl::float2) * 18,
                    dpct::device_to_host);

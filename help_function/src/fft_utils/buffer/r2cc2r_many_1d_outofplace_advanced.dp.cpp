// ===--- r2cc2r_many_1d_outofplace_advanced.dp.cpp -----------*- C++ -*---===//
//
...
bool r2cc2r_many_1d_outofplace_advanced() {
  dpct::device_ext &dev_ct1 = dpct::get_current_device();
  sycl::queue &q_ct1 = dev_ct1.default_queue();
  dpct::fft::fft_engine *plan_fwd;
  plan_fwd = dpct::fft::fft_engine::create();
  float forward_idata_h[26];
  std::memset(forward_idata_h, 0, sizeof(float) * 26);
  ...
  forward_idata_d = (float *)dpct::dpct_malloc(sizeof(float) * 26);
  forward_odata_d = (sycl::float2 *)dpct::dpct_malloc(sizeof(sycl::float2) * 6);
  backward_odata_d = (float *)dpct::dpct_malloc(sizeof(float) * 26);
  dpct::dpct_memcpy(forward_idata_d, forward_idata_h, sizeof(float) * 26,
                    dpct::host_to_device);
  ...
  plan_fwd->commit(&q_ct1, 1, n, inembed, 2, 13,
                   dpct::library_data_t::real_float, onembed, 1, 3,
  ...
  sycl::float2 forward_odata_h[6];
  dpct::dpct_memcpy(forward_odata_h, forward_odata_d, sizeof(sycl::float2) * 6,
                    dpct::device_to_host);

  sycl::float2 forward_odata_ref[6];
  forward_odata_ref[0] = sycl::float2{10, 0};
  forward_odata_ref[1] = sycl::float2{-2.5, 3.44095};
  forward_odata_ref[2] = sycl::float2{-2.5, 0.812299};
  forward_odata_ref[3] = sycl::float2{10, 0};
  forward_odata_ref[4] = sycl::float2{-2.5, 3.44095};
  forward_odata_ref[5] = sycl::float2{-2.5, 0.812299};
  ...
  plan_bwd->commit(&q_ct1, 1, n, onembed, 1, 3,
                   dpct::library_data_t::complex_float, inembed, 2, 13,
  ...
  dev_ct1.queues_wait_and_throw();
  float backward_odata_h[26];
  dpct::dpct_memcpy(backward_odata_h, backward_odata_d, sizeof(float) * 26,
                    dpct::device_to_host);

  float backward_odata_ref[26];
...
#ifdef DEBUG_FFT
int main() {
#define FUNC r2cc2r_many_1d_outofplace_advanced
  bool res = FUNC();

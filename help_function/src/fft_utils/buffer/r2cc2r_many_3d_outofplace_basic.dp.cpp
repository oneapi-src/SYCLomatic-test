...
bool r2cc2r_many_3d_outofplace_basic() {
  ...
  forward_idata_d = (float *)dpct::dpct_malloc(2 * sizeof(float) * 4 * 2 * 3);
  forward_odata_d = (sycl::float2 *)dpct::dpct_malloc(2 * sizeof(sycl::float2) *
                                                      4 * 2 * (3 / 2 + 1));
  backward_odata_d = (float *)dpct::dpct_malloc(2 * sizeof(float) * 4 * 2 * 3);
  dpct::dpct_memcpy(forward_idata_d, forward_idata_h,
                    2 * sizeof(float) * 4 * 2 * 3, dpct::host_to_device);
  ...
  dpct::dpct_memcpy(backward_odata_h, backward_odata_d, sizeof(float) * 48,
                    dpct::device_to_host);

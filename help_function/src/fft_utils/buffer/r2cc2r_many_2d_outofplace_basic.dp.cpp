...
bool r2cc2r_many_2d_outofplace_basic() {
  ...
  forward_idata_d = (float *)dpct::dpct_malloc(sizeof(float) * 2 * 3 * 2);
  forward_odata_d = (sycl::float2 *)dpct::dpct_malloc(
      2 * 2 * sizeof(sycl::float2) * (3 / 2 + 1));
  backward_odata_d = (float *)dpct::dpct_malloc(sizeof(float) * 2 * 3 * 2);
  dpct::dpct_memcpy(forward_idata_d, forward_idata_h, sizeof(float) * 2 * 3 * 2,
                    dpct::host_to_device);
  ...
  dpct::dpct_memcpy(backward_odata_h, backward_odata_d, sizeof(float) * 12,
                    dpct::device_to_host);

...
template <typename T1, typename T2> struct TestCaseStorage {
  T1 a;
  T2 b;
  DotProductAcc<T1, T2> c;
  DotProductAcc<T1, T2> d;
};
...
template <TestType Type, typename T1, typename T2> bool test(const char *msg) {
  using Case = TestCase<Type, T1, T2>;
  using CaseElement = std::remove_cv_t<std::remove_extent_t<decltype(Case::data)>>;
  using ResultT = DotProductAcc<T1, T2>;
  ...
  que.submit([&](sycl::handler &handler) {
    ...
    handler.parallel_for(N, [=](sycl::id<1> i) {
      if constexpr (Type == dp2a_lo)
        res[i] = dpct::dp2a_lo<T1, T2>(src[i].a, src[i].b, src[i].c);
      else if constexpr (Type == dp2a_hi)
        res[i] = dpct::dp2a_hi<T1, T2>(src[i].a, src[i].b, src[i].c);
      else
        res[i] = dpct::dp4a<T1, T2>(src[i].a, src[i].b, src[i].c);
    });
  });

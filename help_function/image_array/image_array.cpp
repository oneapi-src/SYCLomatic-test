...
dpct::image_wrapper<cl::sycl::float4, 3, true> tex43;
dpct::image_wrapper<cl::sycl::float4, 2, true> tex42;

void test_image(sycl::float4* out, dpct::image_accessor_ext<cl::sycl::float4, 2,true> acc42,
                  dpct::image_accessor_ext<cl::sycl::float4, 1,true> acc21) {
  out[0] = acc42.read(16, 0.5f, 0.5f);
  cl::sycl::float4 data32 = acc21.read(16, 0.5f);
  out[1].x() = data32.x();
  out[1].y() = data32.y();
}
...
int main() {
  ...
  texDesc22.set(cl::sycl::coordinate_normalization_mode::normalized);
  ...
  }
  {
    ...
    dpct::get_default_queue().submit([&](cl::sycl::handler &cgh) {
      ...
      auto acc21 = static_cast<dpct::image_wrapper<cl::sycl::float4, 2, true> *>(tex42)->get_access(cgh);
      ...
      cgh.single_task<dpct_kernel_name<class dpct_single_kernel>>([=] {
        test_image(acc_out.get_pointer(),dpct::image_accessor_ext<cl::sycl::float4, 2, true>(smpl42, acc42),
                   dpct::image_accessor_ext<cl::sycl::float4, 1, true>(smpl21, acc21));
      });
    });
  }

...
def dequantize_q2_k_gpu(data, device:str ="cuda", target_dtype = torch.get_default_dtype()):
    ...
    # the best way to fix this is transfer ptr to dequantize_extension instead of Tensor.
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return dequantize_extension.dequantize_q2_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q3_k_gpu(data, device:str ="cuda", target_dtype = torch.get_default_dtype()):
    ...
    # the best way to fix this is transfer ptr to dequantize_extension instead of Tensor.
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return dequantize_extension.dequantize_q3_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q4_k_gpu(data, device:str ="cuda", target_dtype = torch.get_default_dtype()):
    ...
    # the best way to fix this is transfer ptr to dequantize_extension instead of Tensor.
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return dequantize_extension.dequantize_q4_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q5_k_gpu(data, device:str ="cuda", target_dtype = torch.get_default_dtype()):
    ...
    # the best way to fix this is transfer ptr to dequantize_extension instead of Tensor.
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return dequantize_extension.dequantize_q5_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q6_k_gpu(data: np.ndarray, device:str = "cuda", target_dtype = torch.get_default_dtype()):
    ...
    data = np.frombuffer(data, dtype=data.dtype)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return dequantize_extension.dequantize_q6_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)

...
{
    uint32_t tid = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
    ...
    double& duration)
{
    ...
    // allocate device memory
        CUDA_CHECK(
            DPCT_CHECK_ERROR(salt_d = sycl::malloc_device<unsigned char>(
                                 SALT_SIZE, dpct::get_in_order_queue())));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(padding_d = sycl::malloc_device<unsigned char>(
                                 PADDING_SIZE, dpct::get_in_order_queue())));

    // copy to device memory
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_in_order_queue()
                .memcpy(salt_d, salt, SALT_SIZE * sizeof(unsigned char))
                .wait()));
        CUDA_CHECK(
            DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                 .memcpy(padding_d, padding,
                                         PADDING_SIZE * sizeof(unsigned char))
                                 .wait()));
    ...
    dpct::get_in_order_queue().parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, 1024) * sycl::range<3>(1, 1, 16),
                          sycl::range<3>(1, 1, 16)),
    ...
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(salt_d, dpct::get_in_order_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(padding_d, dpct::get_in_order_queue())));

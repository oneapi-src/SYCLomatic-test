	...
	unsigned int const *TS3)
{
    ...
    enc_schedule0 = dpct::byte_level_permute(m0, 0, 0x0123) ^ local_key0;
    enc_schedule1 = dpct::byte_level_permute(m1, 0, 0x0123) ^ local_key1;
    enc_schedule2 = dpct::byte_level_permute(m2, 0, 0x0123) ^ local_key2;
    enc_schedule3 = dpct::byte_level_permute(m3, 0, 0x0123) ^ local_key3;
    ...
    output0[0] = dpct::byte_level_permute(enc_schedule0, 0, 0x0123);
    output1[0] = dpct::byte_level_permute(enc_schedule1, 0, 0x0123);
    output2[0] = dpct::byte_level_permute(enc_schedule2, 0, 0x0123);
    output3[0] = dpct::byte_level_permute(enc_schedule3, 0, 0x0123);
}
    ...
    unsigned int const *TS3)
{
    ...
    **********************************************************************/
        ...
        encrypt(hash0, hash1, hash2, hash3, hash4, hash5, hash6, hash7, a, b, c,
                d, &(schedule00), &(schedule01), &(schedule02), &(schedule03),
                TS0, TS1, TS2, TS3);
        ...
        encrypt(hash0, hash1, hash2, hash3, hash4, hash5, hash6, hash7, a, b, c,
                d, &(schedule04), &(schedule05), &(schedule06), &(schedule07),
                TS0, TS1, TS2, TS3);
        ...
        encrypt(hash0, hash1, hash2, hash3, hash4, hash5, hash6, hash7, a, b, c,
                d, &(schedule08), &(schedule09), &(schedule10), &(schedule11),
                TS0, TS1, TS2, TS3);
        ...
        encrypt(hash0, hash1, hash2, hash3, hash4, hash5, hash6, hash7,
                ((uint32_t *)(macIV))[0], ((uint32_t *)(macIV + 4))[0],
                ((uint32_t *)(macIV + 8))[0], ((uint32_t *)(macIV + 12))[0],
                &(schedule16), &(schedule17), &(schedule18), &(schedule19), TS0,
                TS1, TS2, TS3);
        ...
        encrypt(hash0, hash1, hash2, hash3, hash4, hash5, hash6, hash7,
                ...
                ((uint32_t *)(computedMacIV + 12))[0], &(schedule12),
                &(schedule13), &(schedule14), &(schedule15), TS0, TS1, TS2,
                TS3);
        ...
        encrypt(hash0, hash1, hash2, hash3, hash4, hash5, hash6, hash7,
                schedule28, schedule29, schedule30, schedule31, &(schedule12),
                &(schedule13), &(schedule14), &(schedule15), TS0, TS1, TS2,
                TS3);
...
double attack(char *dname, uint32_t *d_w_words_uint32,
              unsigned char *encryptedVMK, unsigned char *nonce,
              unsigned char *encryptedMAC, int gridBlocks,
              double &duration) try {
    ...
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(
        h_found = sycl::malloc_host<int>(1, dpct::get_in_order_queue())));
    /*
    DPCT1048:1: The original value cudaHostAllocDefault is not meaningful in the
    migrated code and was removed or replaced with 0. You may need to check the
    migrated code.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(h_pswd_char = sycl::malloc_host<char>(
                                    max_num_pswd_per_read * PSWD_NUM_CHAR,
                                    dpct::get_in_order_queue())));
    /*
    DPCT1048:2: The original value cudaHostAllocDefault is not meaningful in the
    migrated code and was removed or replaced with 0. You may need to check the
    migrated code.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(h_pswd_uint32 = sycl::malloc_host<uint32_t>(
                                    max_num_pswd_per_read * PSWD_NUM_UINT32,
                                    dpct::get_in_order_queue())));
    *h_found = -1;
    ...
    CUDA_CHECK(DPCT_CHECK_ERROR(
        dpct::get_in_order_queue()
            .memcpy(h_w_words_uint32, d_w_words_uint32, 4 * sizeof(int))
            .wait()));
    // ------------------------

    // ---- DEVICE VARS are d_vmk, d_vmkIV, d_mac, d_macIV, d_computedMacIV, d_found, d_w_words_uint32, d_pswd_uint32 ----
    CUDA_CHECK(
        DPCT_CHECK_ERROR(d_vmk = sycl::malloc_device<uint8_t>(
                             VMK_FULL_SIZE, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(d_vmkIV = sycl::malloc_device<uint8_t>(
                                    IV_SIZE, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(d_mac = sycl::malloc_device<uint8_t>(
                                    MAC_SIZE, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(d_macIV = sycl::malloc_device<uint8_t>(
                                    IV_SIZE, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(d_computedMacIV = sycl::malloc_device<uint8_t>(
                                    IV_SIZE, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        d_found = sycl::malloc_device<int>(1, dpct::get_in_order_queue())));

    CUDA_CHECK(DPCT_CHECK_ERROR(d_pswd_uint32 = sycl::malloc_device<uint32_t>(
                                    max_num_pswd_per_read * PSWD_NUM_UINT32,
                                    dpct::get_in_order_queue())));
    // d_w_words_uint32 was allocated and populated in evaluate_w_block()/kernel_w_block()
    ...
    CUDA_CHECK(DPCT_CHECK_ERROR(
        dpct::get_in_order_queue()
            .memcpy(d_vmk, encryptedVMK, VMK_FULL_SIZE * sizeof(uint8_t))
            .wait()));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                             .memcpy(d_vmkIV, vmkIV, IV_SIZE * sizeof(uint8_t))
                             .wait()));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        dpct::get_in_order_queue()
            .memcpy(d_mac, encryptedMAC, MAC_SIZE * sizeof(uint8_t))
            .wait()));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                             .memcpy(d_macIV, macIV, IV_SIZE * sizeof(uint8_t))
                             .wait()));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        dpct::get_in_order_queue()
            .memcpy(d_computedMacIV, computedMacIV, IV_SIZE * sizeof(uint8_t))
            .wait()));
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_in_order_queue()
                                    .memcpy(d_found, h_found, sizeof(uint32_t))
                                    .wait()));
	...
	while(true) {
        ...
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_in_order_queue()
                .memcpy(d_pswd_uint32, h_pswd_uint32,
                        num_read_pswd * PSWD_NUM_UINT32 * sizeof(uint32_t))
                .wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(
        ...
        */
        ...
        // copy d_found from device to h_found in host
                CUDA_CHECK(DPCT_CHECK_ERROR(
                    dpct::get_in_order_queue()
                        .memcpy(h_found, d_found, sizeof(unsigned int))
                        .wait()));
                CUDA_CHECK(DPCT_CHECK_ERROR(
                    dpct::get_current_device().queues_wait_and_throw()));
    ...
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(h_found, dpct::get_in_order_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(h_pswd_char, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(h_pswd_uint32, dpct::get_in_order_queue())));
    // free device allocated variables
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(d_vmk, dpct::get_in_order_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(d_vmkIV, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(sycl::free(d_mac, dpct::get_in_order_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(d_macIV, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(d_computedMacIV, dpct::get_in_order_queue())));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(sycl::free(d_found, dpct::get_in_order_queue())));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        sycl::free(d_pswd_uint32, dpct::get_in_order_queue())));
